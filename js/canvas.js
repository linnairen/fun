...
function drawBox(num,x,y){//创建方块图形
	if(typeof(x)!="number"||typeof(y)!="number"){
		return;
	}
	canCtx.fillStyle=color[num];
	canCtx.fillRect(x*125+4,y*125+4,121,121);
	if(typeof(num)=="number"){
		var fontSize=(5-String(num).length)*5+20;
		canCtx.font=fontSize+"px Verdana";
		var trueFont=fontSize-6-(5-String(num).length)*2;//字体真实占用
		var fontX=((121-trueFont*String(num).length)/2)+x*125+4;
		var fontY=((121+trueFont)/2)+y*125+4;
		canCtx.fillStyle="#000000";
		canCtx.fillText(num,fontX,fontY);
	}
}
function RestPo(){//重新摆放方块
	for(var i=0;i<nowPo.length;i++){
		for(var j=0;j<nowPo[i].length;j++){
			if(nowPo[i][j]){
				drawBox(nowPo[i][j],j,i);
				CurNum++;
			}
			else{
				drawBox("0",j,i);
...
function canvasLoad(){//canvas页面加载
	//以下为外边框
	canCtx.beginPath();
	canCtx.lineWidth="4";
	...
	canCtx.beginPath();
	canCtx.moveTo(4,127);
	canCtx.lineTo(500,127);
	canCtx.moveTo(4,252);
	canCtx.lineTo(500,252);
	canCtx.moveTo(4,377);
	canCtx.lineTo(500,377);
	canCtx.moveTo(127,4);
	canCtx.lineTo(127,500);
	canCtx.moveTo(252,4);
	canCtx.lineTo(252,500);
	canCtx.moveTo(377,4);
	canCtx.lineTo(377,500);
	canCtx.stroke();
}
